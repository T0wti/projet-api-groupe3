...
function ExplorePage() {
        ...
        if (q.startsWith('#')) {
          ...
          setPeople(
            userResults.map((u, i) => ({
              id: u.id,
              username: u.username,
              avatarUrl: profileResults[i].status === 'fulfilled' ? (profileResults[i] as PromiseFulfilledResult<any>).value.avatar_url ?? null : null,
            }))
          );
        }
      } catch {
  ...
  }, [authLoading, searchParams, runSearch]);
  ...
  return (
    <main className="w-full border-x border-gray-200 min-h-screen">
      ...
      {/* Loading */}
      {isLoading && (
        <p className="text-center text-gray-400 py-8">{t('pending')}</p>
      )}
      ...
      {!isLoading && !error && searchMode === 'tags' && (
        posts.length === 0
          ? <p className="text-center text-gray-400 py-16">{t('explore_page.no_results_tags')}</p>
          : (
      ...
      {!isLoading && !error && searchMode === 'normal' && activeTab === 'posts' && (
        posts.length === 0
          ? <p className="text-center text-gray-400 py-16">{t('explore_page.no_results_posts')}</p>
          : (
            <section className="px-10 py-5 space-y-5">
              {posts.map((post) => (
                <PostCardAny
      ...
      {!isLoading && !error && searchMode === 'normal' && activeTab === 'people' && (
        people.length === 0
          ? <p className="text-center text-gray-400 py-16">{t('explore_page.no_results_people')}</p>
          : (
              ...
              {people.map((person) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">@{person.username.toLowerCase()}</p>
                  </div>
                </Link>
              ))}
            </section>
